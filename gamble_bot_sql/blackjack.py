...
class Blackjack:
    ...
    def __str__(self):
        msg = [f"**__{self.player_name}'s game with bet amount: ${self.player_bet}__**\n\n"]
        if self.player_bj:
            msg.append(f"**{self.player_name} got a blackjack!**\n")
        else:
            msg.append(f"__{self.player_name}'s hand with value {self.player_hand_value()}:__\n")
        msg.append(self.hand_as_str(self.player_hand))

        if self.dealer_bj:
            msg.append("\n\n**Dealer got a blackjack!**\n")
        else:
            msg.append(f"\n\n__Dealer's hand with value {self.dealer_hand_value()}:__\n")
        msg.append(self.hand_as_str(self.dealer_hand))

        if self.game_state == 0:
            msg.append(f"\n\n__Game results:__\nYou won ${self.player_bet}")
        elif self.game_state == 1:
            msg.append(f"\n\n__Game results:__\nYou lost ${self.player_bet}")
